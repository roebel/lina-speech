...
def process_text_to_audio(out_dir, text_path, num_variants, audio_prompt_path, text_prompt_path, verbose=True):

    if ((audio_prompt_path is None) and (text_prompt_path is not None)
            or (audio_prompt_path is not None) and (text_prompt_path is None)):
        raise ValueError("audio_prompt_path and text_prompt_path must be both None or both not None")
    ...
    wavtokenizer = WavTokenizer.from_pretrained0802(wt_config, wt_ckpt).to(device)

    if text_prompt_path is not None:
        audio, sr = torchaudio.load(audio_prompt_path)
        prompt_audio = convert_audio(audio, sr, 24000, target_channels=1)

        bandwidth_id = torch.tensor([0]).to(device)
        prompt_audio=prompt_audio.to(device)

        _,discrete_code= wavtokenizer.encode_infer(prompt_audio, bandwidth_id=bandwidth_id)
    ...
    with open(text_path, "r") as ft:
        for itext, intxt in enumerate(ft.readlines()):
            txt = intxt.replace("\n", " ")
            txt = "[BOS]" + prompt_txt + txt + "[EOS]"
            txt_code = torch.LongTensor(tokenizer.encode(txt))
            ...
            for ii, x in enumerate(cuts):
                ...
                features = wavtokenizer.codes_to_features(x[0][..., :].to(device))
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Lina-Speech synthesis with prompt or with random speakers and expressivity.")
    ...
    parser.add_argument("-ap", "--audio_prompt_path", type=str, default=None,
                        help="Path to the audio prompt file (Def: %(default)s)")
    parser.add_argument("-tp", "--text_prompt_path", type=str, default=None,
                        help="Path to the text prompt file (Def: %(default)s)")
    ...
    process_text_to_audio(args.out_dir, args.text_path, args.num_variants,
